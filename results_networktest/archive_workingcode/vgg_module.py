...
def visualize_fmaps(output, channels, layer_name):
    if channels==3:
        ...
        for filter_num in range(channels):
            mm = output.cpu().detach().numpy()
            ...
            plt.savefig(
              "/home/kamil/Dropbox/Current_research/ranking/results_networktest/vis/feature_maps/housenums/95.36/%s/%d/%s_batch%d_filternum%d" % (
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        if vis:
            visualize_fmaps(x, 3, "input")
        output = f.relu(self.bn1(self.c1(x)))
        if vis:
            visualize_fmaps(output, 64, "conv1")
