    ...
    for epochfile in sorted(os.listdir(cur_dir)):
        ...
        for lndx in range(num_layers):
            activity = d['data']['activity_tst'][
                lndx]  # K activity for each layer, they're of size 10000x1024, 10000x20, etc.
            pdb.set_trace()
...
for actndx, (activation, vals) in enumerate(measures.items()):
    ...
    for layerndx, layerid in enumerate(GRID_PLOT_LAYERS):
        ...
        plt.plot(epochs, saved_data[activation]['means'][:, layerid] / saved_data[activation]['stds'][:, layerid],
                 'red', label="SNR")
