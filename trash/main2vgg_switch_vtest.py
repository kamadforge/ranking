...
architecture="vgg"
how_many_epochs=200
annealing_steps = float(0.01 * how_many_epochs)
...
save_path="results/cifar/vgg_93.92/switch_init_%.2f, alpha_%.2f" % (alpha, switch_init)
if not os.path.exists(save_path):
    os.mkdir(save_path)
save_textfile="%s/switch_init_%.2f, alpha_%.2f.txt" % (save_path, alpha, switch_init)
BATCH_SIZE=100
model_parameters = '93.92'
print(len(sys.argv))
switch_layer= sys.argv[1] if len(sys.argv)>1 else 'conv2' # only name, need to change the position of the switches manually separately
lr = 0.1
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        if (switch_layer=='conv14'):
            output=self.switch_multiplication(output, Sprime)
        output = f.relu(self.bn14(output))

        output = self.bn15(output)
        if (switch_layer=='conv15'):
            output=self.switch_multiplication(output, Sprime)
        output = f.relu(self.bn15(output))
...
def load_weights():
    if (resume):
        ...
        checkpoint = torch.load('./checkpoint/ckpt_%s.t7' % model_parameters, map_location=lambda storage, loc: storage)
...
def train(epoch):
    ...
    if epoch==1 or epoch==5 or epoch == 10 or epoch == 15 or  epoch == 25 or epoch==49:
        torch.save(S, '%s/%s_alpha%.2f_switchinit%.2f_%s_ep%d.pt' % (save_path, model_parameters, alpha, switch_init, switch_layer, epoch))
