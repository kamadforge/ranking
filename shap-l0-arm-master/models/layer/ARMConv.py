...
class ArmConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True,
        ...
        self.un = torch.Tensor(self.dim_z).uniform_(0, 1)
    ...
    def update_phi_gradient(self, f1, f2):
        ...
        k = opt.k
        if opt.ar:
            e = k * (f2 * (1 - 2 * self.un)).mean(dim=0)  #averaging over the batch
        else:
            e = k * ((f1 - f2) * (self.un - .5)).mean(dim=0) #averaging over the batch
        #print(self.z_phi.grad)
        e[0]=0
    ...
    def sample_z(self, batch_size, neuron_bool):
        #print("num of params: ", self.out_channels)
        ...
        if self.forward_mode:
            ...
            if self.training:
                if self.local_rep:
                    self.un = self.floatTensor(self.dim_z).uniform_(0, 1).expand(batch_size, self.dim_z)
                else:
                    self.un = self.floatTensor(batch_size, self.dim_z).uniform_(0, 1)

                z[self.un < pi.expand(batch_size, self.dim_z)] = 1
                #print(self.un)
                #print(pi.expand(batch_size, self.dim_z))
            ...
            z = self.floatTensor(batch_size, self.dim_z).zero_()
            z[self.un > pi2.expand(batch_size, self.dim_z)] = 1
            if opt.use_t_in_training:
                z[pi.expand(batch_size, self.dim_z) < opt.t] = 0
        return z.view(batch_size, self.dim_z, 1, 1)

    def forward(self, input_, neuron, neuron_bool):
        """ forward for fc """
        ...
        z = self.sample_z(output.size(0), neuron_bool)
